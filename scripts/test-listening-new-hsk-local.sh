...
printf '%s\n' '[1/27] Kiểm tra cú pháp Listening'
...
printf '%s\n' '[15/27] Browser test audio controls và thanh nổi'
python3 scripts/test-listening-layout-browser.py

printf '%s\n' '[16/27] Browser test chép nguyên và sửa con trỏ'
python3 scripts/test-listening-full-dictation-browser.py

printf '%s\n' '[17/27] Browser test LDSN schema chung, resume và New HSK 15 bài'
python3 scripts/test-listening-sources-browser.py

printf '%s\n' '[18/27] Browser test nhập JSON, XLSX, CSV và TXT'
python3 scripts/test-import-core-browser.py

printf '%s\n' '[19/27] Browser test 课 – HSK & Giáo trình trong tab Thẻ'
python3 scripts/test-flashcard-curriculum-browser.py

printf '%s\n' '[20/27] Browser test Thẻ theo bài, AI Prompt Builder và Chép nhiều câu'
python3 scripts/test-patch2abc-browser.py

printf '%s\n' '[21/27] Browser test Dán kết quả AI trên mobile'
python3 scripts/test-ai-paste-browser.py

printf '%s\n' '[22/27] Shared Matching engine, Nối chữ và chạm để nghe'
node tests/test_matching_engine.js

printf '%s\n' '[23/27] Browser test Nối chữ mobile cho Nghe và Thẻ'
python3 scripts/test-matching-browser.py

printf '%s\n' '[24/27] Browser test Nối chữ mobile cho LDSN1-4'
python3 scripts/test-ldsn-matching-browser.py

printf '%s\n' '[25/27] Browser test bố cục Nối chữ thích ứng 360/390/430'
python3 scripts/test-matching-adaptive-browser.py

printf '%s\n' '[26/27] Mobile activity shell, pinyin toggle và cache contract'
