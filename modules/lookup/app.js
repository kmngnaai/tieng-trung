...
const UNIFIED_BASE_URL = resolveAssetUrl('../hanzi-stroke/data/learning/unified-lookup/all-sources/');
...
const SINGLE_CHAR_INDEX_URL = '../hanzi-stroke/data/learning/character-enrichment/hsk1-3-single/index.json';
const SINGLE_CHAR_BASE_URL = '../hanzi-stroke/data/learning/character-enrichment/hsk1-3-single/';
const SINGLE_CHAR_RADICAL_INDEX_URL = '../hanzi-stroke/data/learning/character-enrichment/hsk1-3-single/radical-character-index.json';
const LOCAL_CHAR_BASE = '../hanzi-stroke/data/chars/';
const HSK_LOOKUP_URL = '../hanzi-stroke/data/learning/hsk/hsk_flashcard_lookup.json';
const HSK_LEVEL_URLS = [1, 2, 3].map(level => `../hanzi-stroke/data/learning/hsk/hsk_${level}.json`);
const WORD_INDEX_URL = '../hanzi-stroke/data/learning/word-enrichment/hsk1-3/index.json';
const WORD_SEARCH_INDEX_URL = '../hanzi-stroke/data/learning/word-enrichment/hsk1-3/search_index.json';
...
function lookupUrlFor(view = state.traView, target = '') {
  ...
  if (view === 'detail' && clean(target)) url.searchParams.set('q', clean(target));
...
async function openLookupLanding() {
...
function initTraHistoryBoundary() {
  ...
  if (!current || current.namespace !== TRA_HISTORY_KEY) {
    history.replaceState(traHistoryState('landing'), '', lookupUrlFor('landing'));
  }
...
function hasVietnameseMeaning(item) {
  const meaning = clean(item?.meaningVi);
  if (!meaning) return false;
  const normalized = normalizeSearchText(meaning);
  return ![
    'chua co nghia tieng viet trong kho hien tai',
    'chua co nghia phu hop',
    'chua co nghia local'
  ].includes(normalized);
}

function catalogMatches(item, filter, query) {
  if (!hasVietnameseMeaning(item)) return false;
...
function catalogResultCard(item) {
  ...
  return `<button type="button" class="catalog-result-card" data-catalog-target="${escapeHtml(item.target)}">
    ...
    <span class="catalog-result-body"><strong>${escapeHtml(item.pinyin || '')}</strong><span>${escapeHtml(item.meaningVi)}</span></span>
...
function adaptUnifiedRecord(raw) {
  ...
  if (single) {
    ...
    return {
      ...
      characterInfo: { strokeCount: raw.strokeCount ?? raw.characters?.[0]?.strokeCount ?? null, structureVi: '', formationTypeVi: '', radical: adaptUnifiedRadical(raw.radical) },
      components,
      ...
      sources, review: { status: raw.dataTier ? `Mức ${raw.dataTier}` : 'unified-source-backed', warnings: [] }
    };
  }
  return {
...
async function loadNormalizedWordRecord(word) {
  ...
  if (!path) return null;
  const raw = await safeFetchJson(`../hanzi-stroke/data/learning/word-enrichment/hsk1-3/${path}`, null);
  if (!raw) return null;
...
async function searchExistingData(query) {
  ...
  const q = normalizeSearchText(query);
  const qPinyin = normalizePinyinSearch(query);
  if (!q && !qPinyin) return [];
...
function openGrammarDetail(index) {
  ...
  dialog.innerHTML = `<div class="grammar-dialog-shell">
    ...
    <div class="grammar-dialog-content">
      ...
      ${examples.length ? `<section class="grammar-popup-examples"><div class="grammar-popup-examples-head"><h4>Ví dụ</h4><span>${examples.length} câu</span></div><div class="grammar-popup-example-list">${examples.map((row, i) => `<article class="grammar-popup-example-card"><span class="grammar-popup-example-index">${String(i + 1).padStart(2, '0')}</span><div class="grammar-popup-example-main"><div class="grammar-popup-example-han-row"><strong>${escapeHtml(row.chinese)}</strong><button class="grammar-example-audio icon-audio-btn" type="button" data-speak="${escapeHtml(row.chinese)}" aria-label="Nghe câu ví dụ">${audioIcon()}</button></div>${row.pinyin ? `<em>${escapeHtml(row.pinyin)}</em>` : ''}${row.meaningVi ? `<span>${escapeHtml(row.meaningVi)}</span>` : ''}</div></article>`).join('')}</div></section>` : ''}
      ${!item.syntax && !item.explanationVi && !item.usageNoteVi && !examples.length ? '<p class="grammar-popup-empty">Chủ điểm này chưa có nội dung chi tiết.</p>' : ''}
    </div>
  </div>`;
...
function writingPanel(data) {
  ...
  return `<div class="writing-char-tabs" aria-label="Chọn chữ luyện viết">${chars.map((char, i) => `<button type="button" class="writing-char-tab ${i === 0 ? 'active' : ''}" data-writer-char="${char}">${char}</button>`).join('')}</div>
    ...
    <a class="open-stroke-link" id="openStrokeLink" href="../hanzi-stroke/index.html?study=writing&word=${encodeURIComponent(chars[0])}">Mở đầy đủ trong Học →</a>`;
}
...
function render(data) {
  ...
  const rawFormation = data.characterInfo?.formationTypeVi || '';
  const formation = ['chưa chuẩn hóa', 'not-available', 'unknown'].includes(String(rawFormation).trim().toLowerCase()) ? '' : rawFormation;
